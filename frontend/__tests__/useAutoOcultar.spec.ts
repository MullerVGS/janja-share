...
describe('useAutoOcultar', () => {
  ...
  it('mover o ponteiro traz de volta e reinicia a contagem', () => {
    ...
    expect(result.current).toBe(true) // ainda dentro da nova janela de 2600 ms

    act(() => vi.advanceTimersByTime(600))
    expect(result.current).toBe(false) // 2600 ms desde o último movimento
  })
  ...
  it('destravar recomeça o relógio do zero, sem herdar o tempo já passado travado', () => {
    ...
    act(() => vi.advanceTimersByTime(600))
    expect(result.current).toBe(false) // 2600 ms desde o destravar
  })
